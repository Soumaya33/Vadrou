...
async function insertLieu(lieu) {
  const row = {
    ...
    categorie: [lieu.categorie],  // tableau PostgreSQL natif, pas JSON.stringify
  ...
  for (const commune of COMMUNES) {
    ...
    for (const { categorie, googleTypes } of CATEGORIE_MAP) {
      ...
      for (const place of places) {
        ...
        // Exclure les types non pertinents pour les familles
        const primaryType = place.primaryType || '';
        const TYPES_EXCLUS = [
          'night_club', 'bar', 'liquor_store',
          'physiotherapist', 'doctor', 'hospital', 'pharmacy',
        ...
        // Exclure sur mots-clés dans le nom
        const nomLower = (place.displayName?.text || '').toLowerCase();
        const NOMS_EXCLUS = [
